...
## 4. 安全扫描记录
- 扫描工具（如 Mimosa）报出的**缺陷**、**待改进点**、以及**已修复的问题**，都必须记录到 `docs/ISSUES.md`：
  - 真实缺陷 → 按 `BUG-<n>` 建档并归档到「已修复缺陷」表（注明修复位置与关闭日期）。
  - 需改进项 / 评估为误报或协议性约束的扫描标记 → 记录到「改进建议」区（`IMPROVE-<n>`），写明判定依据与建议（如客户端豁免、换环境变量显式声明等）。
  - 同时在 ISSUES.md 头部更新 `updated`、`entries`、`fixed`、`pending` 计数。

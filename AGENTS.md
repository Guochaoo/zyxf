...
- 发现的**缺陷**、**待改进点**、以及**已修复的问题**，都必须记录到 `docs/ISSUES.md`：
  ...
  - **归档表只作索引**（编号 / 严重度 / 类别 / 标题 / 位置 / 日期），一行一条，**不写长说明**。设计取舍、踩坑与验证方法写在对应代码的注释里，并在该注释中带上编号（便于 `grep -rn "BUG-54" backend/src` 反查）；纯文档/配置类条目的依据写在 commit message 里。
  - **待处理条目**保持简短：现状 / 影响 / 改法 / 验收各一行，说清「能不能复现、改哪里、怎么算修好」即可。

...
- **在弱服务器上跑的重活必须能被闸门限制**：目标部署是「单核小内存」机器，所以任何「把整个对象读进内存」的操作都要有上限（本处是 `INDEX_MAX_FILE_MB`，在**下载之前**按 `files.size` 判断）。实测不设限时 100MB+ 的教材能把常驻内存推到近 1GB，且高水位不回落；带闸门后平稳在 100MB 量级。上限值直接决定服务器 RAM 需求，改默认值要同步 `docs/DEPLOY.md §6.5`。

...
- **建档**：发现的缺陷、待改进点、以及评估为误报/协议性约束的扫描标记，都要**开 issue**（`gh issue create`），不要只留在聊天或提交说明里。
...
- **修复**：commit message 里写 `closes #<issue>`（如 `fix(frontend): …（closes #50）`），合并后 GitHub 自动关闭；**不在代码里回填历史编号注释**。

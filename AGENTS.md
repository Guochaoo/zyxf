...
> 缺陷与改进追踪在 **GitHub Issues**（<https://github.com/Guochaoo/zyxf/issues>），仓库内不再维护清单文档（§4）。
...
仓库内的文档（`README.md`、`docs/DEPLOY.md`、`docs/DESIGN.md`）必须与代码保持一致；**缺陷与改进追踪在 GitHub Issues**（见 §4），仓库内不再维护清单文档。

- **触发时机**：每次代码改动若影响文档描述的行为、路径、端口、断点、环境变量、token、目录或组件，就必须同步更新对应文档。
  - 高频影响点：`frontend/src/test` 路径、issue 编号引用（`#NN`，以及迁移前的 `BUG-<n>` / `IMPROVE-<n>`）、tailwind token（如 `shadow-card` 是否仍存在）、右栏 `300px`/`lg` 断点、登录入口位置、`rb-btn-dark` 圆角等。
...
## 4. 缺陷与改进追踪（GitHub Issues）

> 追踪载体是 **GitHub Issues**：<https://github.com/Guochaoo/zyxf/issues>。
> 2026-09-20 之前用仓库内 `docs/ISSUES.md`（`BUG-<n>` / `IMPROVE-<n>` 编号）；该文件已删除，**已归档的 151 条不迁移**
> （需要时用 `git log --all -- docs/ISSUES.md` 翻历史），当时待处理的 10 条已迁到 issue 并在标题里保留原编号前缀。

- **建档**：**agent 自己**发现的缺陷、待改进点、以及评估为误报/协议性约束的扫描标记，都要**开 issue**（`gh issue create`），不要只留在聊天或提交说明里。
  - **例外——人类直接指出并要求修的，不开 issue**：人类在对话里点名的问题（「hover 这种右侧边缘的格子有些会被挡住」）由人类自己掌握上下文，直接修即可；开 issue 反而多一份要维护的副本。
  - 类型标签：缺陷 `bug`、改进/建议 `enhancement`（沿用 GitHub 默认标签）。
  - 严重度标签：`P0` 功能错误 / 崩溃 / 安全 / 阻塞发布 · `P1` 性能退化 / 逻辑隐患 / 有成本或安全影响 · `P2` 健壮性 / 规范 / 边缘 case / 轻微改进。
  - 范围标签：`前端` / `后端` / `工程` / `安全` / `测试`；有意的取舍或暂不动再加 `暂缓`。
  - **正文写全背景**（现状 / 影响 / 修法 / 验证），保留具体数值、复现路径、约束与踩坑：目标是「以后接手的人不必重新调研就能动手」。定位写「文件 + 关键字」，别只写行号（会漂移）。
- **修复**：commit message 里写 `closes #<issue>`（如 `fix(frontend): …（closes #50）`），合并后 GitHub 自动关闭；**不在代码里回填历史编号注释**。人类直接指出的修复没有 issue 号，正常写 `fix(...)` 即可，不要为了凑 `closes` 去补建 issue。
- **注释只留在「不看就容易改错」的地方**：仅当修复的原因/约束从代码本身看不出来时，在修复处留**一行**注释，形如 `// #54：校验必须和 UPDATE 同事务，否则并发能写出环`；一行说不完就不写（背景属于 issue 与 commit message），不复述代码在做什么。
- **迁移前的旧编号**：代码里 2026-09-20 之前写的 `BUG-<n>` / `IMPROVE-<n>` 注释保留原样；迁移过来的条目标题带同样前缀，可用 `gh issue list --search "BUG-94"` 查到，已归档的只能从 `git log` 里翻。
- **收尾**：修完在 issue 里补一行验证结论（跑了哪些测试、实测数据/截图），再关闭。

...
- **归档表是索引，不是文档**：只留编号 / 严重度 / 类别 / 标题 / 位置 / 日期。**为什么这样修、踩过什么坑、怎么验证**写在代码注释里（注释带编号，`grep -rn "BUG-54" backend/src frontend/src` 即可反查）；纯文档/配置类条目看 commit message。`处置要点` 列已废弃，不要再往归档表里补长说明。

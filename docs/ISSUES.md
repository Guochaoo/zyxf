...
```yaml
更新日期: 2026-09-11
条目总数: 123        # 缺陷 89 + 改进 34
待处理: 7            # 缺陷 2 + 改进 5（26 暂缓；31/32 已建档、待决策后修；33 为可访问性权衡；34 为视觉一致性）
已归档: 116          # 缺陷 87 + 改进 29
# 本批（设计规范审计）：把 docs/DESIGN.md 逐段中文化并逐条对照代码校正，删掉未实现的 Vercel 通用内容。
# 审计中新发现 BUG-91（热力图星期标签错位一天）、BUG-92（侧栏时长/缓动的类名被模板插值拼掉，退化成 150ms）
# 与 IMPROVE-34（聊天未知类型徽章被品牌色覆盖规则染成纯黑）；本批按「只改文档」的口径只建档、未动代码。
...
上一轮审计新发现的 11 条缺陷已全部处置并归档（BUG-54～61、64/66/67/79/85～90 见 [2.1 已修复缺陷](#21-已修复缺陷)（87））；本轮（设计规范审计）新发现 **2 条**（BUG-91、BUG-92），只建档、未改代码。
...
#### BUG-91 · 下载热力图的星期标签比格子错开一天（周一开头的网格配了周日开头的字典）
...
#### BUG-92 · 左栏开合的时长 / 缓动被写成模板插值类名，Tailwind 扫不到 → 实际退化成默认 150ms

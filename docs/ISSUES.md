...
- **现状**：图谱的语义边原先只来自**文件名**（IMPROVE-38）。本轮补上内容级：`text_extractions`（正文，全存）/ `file_embeddings`（512 维 float32 BLOB）/ `index_jobs`（队列）/ `index_usage`（每日配额）四张表，上传与 `/api/sync` 后自动入队，后台单并发 worker 下载 → 抽正文 → 本地嵌入，图谱新增「内容视图」档（默认），边来自 `GET /api/index/semantics`（每节点 top-5、阈值 0.72，后端算完只回边与标签）。
...
- **实测覆盖率（等距抽样 40 个 PDF + 各格式最大 10 个，非按大小抽样以免偏）**：

  | 类别 | 估计占比 | 处理 |
  |---|---|---|
  | PDF 有文本层 | 526 个中约 **80%** | 正常入内容视图 |
  | PDF 扫描件 | 约 **20%**（抽样命中 4 本大部头：复变函数 365 页、数据结构 479 页、高数下 379 页、线性代数习题集 488 页） | `image_only`，只留名称层语义 |
  | docx/pptx 图片版 | 抽样 10 个里各 2–3 个（如 `传热学往年题目.docx` 2.2 MB 但文本节点为 0） | `image_only` |
  | `.doc` / `.ppt` | **191 个（22%）** | `unsupported`：老二进制复合文档没有纯 JS 解析器 |

  合起来**只有约 49% 的资料能进内容视图**。本轮的取舍（人类已确认）：不做 OCR、老格式不做，但把位置留好——`doc_kind` 里已预留 `pdf_ocr`，下一轮接本地 Tesseract 时不用改表结构。**不接 OCR 的后果要正视**：抽样里那几本扫描教材正是复习主力，它们在内容视图里等于不存在。
...
- **已知不足**：① 扫描件与老格式（约 51%）没有内容语义，想要覆盖需 OCR，成本见上；② 簇标签目前取「簇内最高频目录名」而不调 LLM（本轮按人类选择省成本），因此同一目录下不同主题的资料会共用一个图例名；③ 相似度是**同步**在内存里暴力算的（850×512 float32 ≈ 1.7 MB，全量点积约 44 万次乘加），库规模上到几万条需要换 ANN 索引（hnsw / sqlite-vec）；④ 向量只存整篇均值、没有分块，长教材里「某一章讲了什么」这种细粒度检索还没有（搜索混合排序留到下一轮）。
- **验证**：后端 270 条 / 前端 172 条全绿（新增 `textExtract.test.js` 13 条、`indexPipeline.test.js` 11 条、图谱渲染与 `buildVectorGraph` 9 条）。真实库端到端：850 个文件入队后 worker 正常消化，`/api/index/status` 可见各 `doc_kind` 分布与失败清单。

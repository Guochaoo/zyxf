...
> 留在「新代码 + 缺依赖」状态（旧进程仍在跑所以站点没挂，但**一重启后端就起不来**，且自动
> 回滚只在健康检查失败时触发、这次更早就挂了）。已由人工把整批 revert 出 `dev` 与 `main`
> 并恢复线上。
>
...
#   本地预索引只搬四张表两条路线，见 IMPROVE-42 与 DEPLOY §6.5。
# 本批（上传之后的链路）：核对「上传新文件怎么办」时发现两处脱节——分类缓存是整库单行，
#   上传 1 个文件会重新 LLM 命名全部 53 个学科（273 秒、还在请求里同步跑）；前端分类缓存
#   永不失效且上传后没通知图谱（新文件在图谱里永远看不见）。已改为按学科缓存（新增只重建
#   所在学科）、上传后广播变更、索引期间显示进度并自动重拉，见 IMPROVE-41。
...
- **已知不足**：① 大类就是顶层目录，所以**目录起得乱的库，大类也会乱**（本库里「各类往年题」「思政类」这类归档目录本身不是学科，它们下面按内容分出的细分仍是对的，但大类名不理想）；② k-means 是硬聚类，一个文件只能属于一个细分（跨学科资料如「入学考试数学+物理」会被塞进其中一个）。

#### IMPROVE-41 · 上传新文件之后：分类原先会整库重算 + 前端永不刷新（两个脱节）
**影响范围**：`backend/src/semanticTaxonomy.js` · `backend/src/db.js` · `backend/src/routes/indexing.js` · `frontend/src/components/KnowledgeGraph.jsx` · `frontend/src/pages/BrowsePage.jsx`（前后端 · 功能 / 正确性）

- **现状（上线后按「上传新文件怎么办」逐环节核对时发现）**：整条链路原本只有「上传 → 入队 → worker 抽取嵌入」是通的，后面的分类与前端刷新有两处脱节：
  1. **分类缓存是整库单行**（`taxonomy_cache` 一个 `id=1` 行 + 整库指纹）。上传 1 个文件 → 整库指纹变化 → **53 个学科全部重新 LLM 命名**（实测约 50 次调用 / **273 秒**），而且是在 `GET /taxonomy` 请求里**同步**跑完才返回——用户会看到一个转很久的请求。k-means 本来就按学科独立，整库缓存是设计错误。
  2. **前端分类缓存永不失效**：`KnowledgeGraph` 的模块级 `kgTaxonomyInflight` 只在失败时清空；而 `UploadDialog.onDone` 只调了 `refresh()`（刷文件列表），**没有 `notifyFoldersChanged()`**，图谱那边既收不到通知、自己的缓存也不会过期 → 新文件在图谱里永远看不见。
- **修法**：① 缓存拆成 `taxonomy_subjects`（**一个学科一行**，指纹只覆盖该学科的「文件 id + 向量时间」），`deriveTaxonomy` 逐学科命中/重算并返回 `{rebuilt, reused}`；实测**新增 1 个文件只重建 1 个学科、复用 52 个**（0.1 秒完成，不再触发 LLM）。② 索引是异步的，所以分类请求额外回 `pending`（待索引数），前端据此**每 5 秒自动重拉一次**（上限 12 次 ≈ 1 分钟，之后交给手动刷新），并在图谱区右下角显示「正在建立内容索引（N 个待处理）…」。③ 上传成功后调 `notifyFoldersChanged()`，图谱监听 `folders-changed` 清缓存并重拉。
- **顺带修的性能问题**：`deriveTaxonomy` 原先对每个文件单独查一次向量（N+1），现在 `file_embeddings` 一次读出（含 BLOB），只有真要重算的学科才解析成 Float32Array；`subjectOf` 也从「每文件上溯查库」改成一次性把 folders 读进内存做上溯。
- **验证**：后端 276 条（新增 `semanticTaxonomy.test.js` **6 条**：首次全量各算一次 / 二次全命中 / **新增一个文件只重建所在学科** / 删除同理 / 无向量的文件不进分类且不让学科失效 / 同样输入结果完全一致）、前端 164 条（新增 1 条：`folders-changed` 后重新请求 + 索引提示的出现与消失）全绿。真实库验证：全量 53 学科 0.1 秒建缓存 → 再次请求 0 重算 → 模拟新增 1 个文件后**只重建 1 个学科、复用 52 个**。

#### IMPROVE-42 · 部署到单核小内存服务器：解析大文件的内存尖峰（已加体积闸门）
**影响范围**：`backend/src/indexPipeline.js` · `backend/src/textExtract.js` · `.env.example` · `docs/DEPLOY.md` · `README.md`（后端 · 性能 / 运维）

- **背景**：人类要部署到「单核双线程、性能很差」的云服务器上，问有没有问题。逐项实测后结论是**CPU 不是瓶颈、内存才是**：
  - **CPU 没问题**：实测索引 151MB PDF 期间，`/api/health` 的 p50 **1ms** / p95 **2ms**——pdfjs 逐页 `await`、ONNX 推理走原生线程池（`intraOpNumThreads: 1`），都不占 JS 线程。网站在索引期间照常响应。
  - **嵌入很便宜**：29ms/文件（全库 850 个约 **25 秒** CPU），弱服务器乘 3~4 倍也只有 1~2 分钟。
  - **内存是问题**：不设限时索引那几本 100MB+ 教材，RSS 从 872MB 涨到**峰值 976MB**，且**高水位不立即回落**。而峰值增量**不是文件大小的固定倍数**——实测 42MB 文件 +188MB（4.4×）、25MB 文件 **+331MB（13.3×）**，取决于 PDF 页数与复杂度（`电路 第6版 笔记和课后习题` 这类页数巨大的最费）。
- **修法**：加**单文件体积闸门** `INDEX_MAX_FILE_MB`（默认 50）。超限的文件**在下载之前就跳过**（`runJob` 里先看 `files.size`），只记一条 `doc_kind = too_large`，并清掉可能存在的旧向量；在 `status` 的 `totals.kinds` 里可见，不静默失踪。实测同一批大文件：**RSS 平稳在 104MB**（此前峰值 976MB）。本库 >50MB 的只有 **29 个（3%）**，代价可控；想覆盖就调大该值（同时要加内存）。
- **另外两条部署优化（写进 DEPLOY §6.5）**：① `onnxruntime-node` 里 **240MB 是别的平台**的原生库（darwin 84MB / win32 116MB / linux-arm64 24MB），Linux x64 服务器只需保留 43MB，给出删除命令；② 弱服务器可以走「**本地预索引、只把四张索引表导到服务器**」的路线（`text_extractions` / `file_embeddings` / `index_jobs` / `taxonomy_subjects`），服务器启动即用、不必跑首次全量索引——⚠️ 只能导这四张表，整库覆盖会清掉 `users` 与 `download_logs`。
- **已知不足**：体积闸门是「下载前」唯一的廉价信号，但它只是**页数/复杂度的粗略代理**——同样 25MB，页数多的可能比 42MB 的更费内存。要更准得先下载再按页数判断，那时内存已经花了；彻底的做法是把抽取放进独立子进程并限制其内存（进程崩了不影响主服务），本轮没做。
- **验证**：后端 289 条（新增 2 条：**超限文件不下载不解析**——故意不放 OSS 对象，若闸门失效会因 `NoSuchKey` 失败；超限替换后**清掉旧向量**）全绿；真实库实测闸门前后 RSS 976MB → 104MB。
- **上线后立刻暴露的一个 bug（已修）**：首页内容视图显示「索引里没有可用资料」，而索引其实是好的。原因是学科节点的存在被写成依赖「**该学科的文件在当前范围内**」——首页的局部范围只有目录、没有文件（`localSubgraph(0)` = f0 + 59 个顶层目录），于是 53 个学科全被跳过。**判据改成「该学科目录在不在范围内」**：学科节点代表整个学科（`size` 取分类总数），细分/文件才按文件是否在范围内决定；首页因此是一张学科地图，点进学科才展开细分。顺带把空态拆成两种文案（整库为空 / 此处为空），先前那句话在小文件夹里会误导。全库扫描 59 个目录：54 个有分类节点、5 个是纯扫描件目录（正确显示提示），0 处误报为空、0 悬空边、0 重复 id。
- **验证**：后端 270 条 / 前端 163 条全绿（图谱 17 条：建图纯函数 8 条 + 渲染 5 条 + 局部子图回归 6 条）。真实库端到端：`deriveTaxonomy` 产出 **53 个大类 / 84 个细分 / 85% 有名字**，用真实数据跑 `buildTopicGraph`：全库 653 节点 / 652 边 / **0 重复 id / 0 悬空边 / 分类覆盖的 518 个文件一个不缺**，根的第一层就是「线性代数 / 生命科学基础 / 各类往年题 / 高数上复习助手 / 大学计算机基础-大计基 / 思想政治-思修 …」，首页显示 53 个学科节点，进入「线性代数」展开出「期末真题汇编 / 复习辅导资料 / 试题资料合集 / 习题与复习」。

...
```yaml
更新日期: 2026-09-11
条目总数: 132        # 缺陷 93 + 改进 39
待处理: 9            # 缺陷 2 + 改进 7（26 暂缓；31/32 已建档、待决策后修；33 为可访问性权衡；34 为视觉一致性；38 名称层语义；39 内容级语义索引，已落地）
已归档: 123          # 缺陷 91 + 改进 32
# 本批（内容级语义索引）：新增正文抽取（PDF/OOXML/纯文本）+ 本地 bge-small-zh 向量 + 索引队列，
#   图谱默认档改为「内容视图」；实测覆盖率与 OCR 取舍写进 IMPROVE-39，模型获取与排错写进 DEPLOY.md §6。
# 本批（知识图谱语义层）：图谱原先只有目录层级边，按人类选择叠加「名称层语义聚类」——虚线语义边、
#   按簇上色、图例可点，默认开启且能一键切回目录视图；四个阈值全用本地真实库（908 节点）校准，
#   翻车点与两条填词表教训写进 IMPROVE-38，簇色/图例规范写进 DESIGN.md §4。
...
#### IMPROVE-38 · 知识图谱只有目录层级、没有语义组织；名称层分词要避开四个实测过的翻车点
...
- **现状**：图谱的语义边原先只来自**文件名**（IMPROVE-38）。本轮补上内容级：`text_extractions`（正文，全存）/ `file_embeddings`（512 维 float32 BLOB）/ `index_jobs`（队列）/ `index_usage`（每日配额）四张表，上传与 `/api/sync` 后自动入队，后台单并发 worker 下载 → 抽正文 → 本地嵌入，图谱新增「内容视图」档（默认），边来自 `GET /api/index/semantics`（每节点 top-8、后端起连线阈值 0.6，算完只回边与标签，不把向量交给前端）。
...
- **实测覆盖率（本机真实库 850 个文件跑完全量索引；另做过等距抽样校准，避免按大小抽样带来的偏差）**：
  ...
  | `text`（txt） | 8 | 1% | 同上 |
  | `image_only`（扫描件 / 图片版 Office） | 122 | 14% | 只留名称层语义 |
  | `unsupported`（.doc/.ppt/压缩包/图片） | 216 | 25% | 只留名称层语义 |
  | 失败 | 3 | — | 记 `last_error`，不影响其他文件 |

  **能进内容视图的是 61%（518/850）**，剩下 39% 靠名称层语义兜住（比方案阶段按抽样估的 49% 好一些）。全量抽取耗时约 25 分钟（单并发，含 154 MB 的模电课本）。本轮取舍（人类已确认）：不做 OCR、老格式不做，但把位置留好——`doc_kind` 已预留 `pdf_ocr`，下一轮接本地 Tesseract 不用改表结构。**不接 OCR 的后果要正视**：抽样里那几本扫描教材（复变函数 365 页、数据结构 479 页等）正是复习主力，它们目前只能靠名称层语义。
- **成簇判据的标定过程（不记下来一定会踩）**：单靠相似度阈值**解决不了链式串簇**——518 个文件 2943 条边里，0.78 时最大簇 125 个成员（数学类试题资料靠「试卷/题目/答案」这一文体逐级串起来）、0.82 时仍有 64 个且纯度只有 25%。改成「阈值 ≥0.84 **且互为 top-4 最近邻**」后最大簇降到 17，且 线性代数 / 思政 / 概率论 / 流体力学 / 生命科学 这些真主题保持 100% 纯度。原理：桥接文件不再能单向把两团拉近——它必须也在对方的前 4 名里，而弱连接通常不是相互的。这一判据与名称层（最大簇 57）已是同一量级，内容层的簇纯度明显更高。
...
- **降级链（都验证过）**：模型文件缺失 → `isEmbeddingEnabled()` 为 false，只抽正文、不出向量，进程不报错、上传不受影响，`/api/index/status` 里给出原因；前端内容视图在数据未就绪时提示「正在读取内容索引…」、索引为空时提示「可切到名称视图」，都不白屏。
- **已知不足**：① 扫描件与老格式（39%）没有内容语义，想要覆盖需 OCR，成本见上；② 簇标签取「簇内最高频目录名」而不调 LLM（本轮按人类选择省成本），因此同一目录下不同主题的资料会共用一个图例名；③ 相似度是**同步**在内存里暴力算的（518×512 float32 ≈ 1 MB，全量点积约 27 万次乘加；向量缓存已改成一次读进**一块** ArrayBuffer 并用 subarray 切视图，避免高频请求持续制造垃圾），库规模上到几万条需要换 ANN 索引（hnsw / sqlite-vec）；④ 向量只存整篇均值、没有分块，长教材里「某一章讲了什么」这种细粒度检索还没有（搜索混合排序留到下一轮）。
- **验证**：后端 270 条 / 前端 173 条全绿（新增 `textExtract.test.js` 13 条、`indexPipeline.test.js` 11 条、图谱渲染与 `buildVectorGraph` 10 条）。真实库端到端：850 个文件入队后 worker 正常消化（847 done / 3 failed / 0 卡住），`/api/index/status` 可见各 `doc_kind` 分布与失败清单；`/api/index/semantics` 返回 518 文件 / 2943 边，成簇后 61 个多成员簇、最大 17，人工核对簇内容正确（生命科学 23/23、思政 15/15、医用有机化学 11/11、材料力学 7/7、软件体系结构 7/7 均为整簇）。

...
```yaml
更新日期: 2026-09-11
条目总数: 131        # 缺陷 93 + 改进 38
待处理: 8            # 缺陷 2 + 改进 6（26 暂缓；31/32 已建档、待决策后修；33 为可访问性权衡；34 为视觉一致性；38 为知识图谱语义层，已落地）
已归档: 123          # 缺陷 91 + 改进 32
...
- **已知不足**：① 纯名称层对同一门课的不同写法（「线代」vs「线性代数」）不能归一，靠文件夹与单字 token 兜底；② 局部视图里 f0/顶层文件夹多数自成一簇（文件夹之间名称差异大），簇色主要体现在文件节点上；③ 停用字表是经验值，换库/换学科要按上面四条重新验证。
- **验证**：`npm test` 全绿（167 条，其中图谱 21 条：分词/阈值/成簇/配色稳定/不污染入参 + 渲染层断言图例与虚线语义边）。真实库实测：908 节点 → 930 条语义边 / 92 个多成员簇 / 最大簇 51（医用有机化学课件）、28（大物）、27（复变积变），主题归并已可用。

...
- 账号条：**通栏贴底的矩形条**——宽 = 面板宽（`--sm-panel-w`）、左右下三边与面板边缘齐平、无圆角，底色沿用原胶囊的 `--field`；内容贴条的两端（头像左边距 = 图标组右边距 = `1rem`），不再跟着列表的 `2em` 内容栏。未登录时是中立条（图标头像 + 「未登录 / 游客」文案），点整条进登录；已登录显示用户名 + 角色，右侧两个图标操作——齿轮打开全局设置，另一个是登录 / 退出（配色见 §2）

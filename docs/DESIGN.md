...
| `1024px`（`lg`） | 三栏 docs 布局生效（左栏 250px + 右栏 300px）、Dashboard 12 栅格开始分栏；反向就是「`lg` 以下」——左右栏整栏隐藏（不做内联回退），改用移动端细品牌行（`h-14`）+ 92% 宽的搜索行 + 菜单导航；`StaggeredMenu` 自己也有一层 `@media (max-width: 1100px)` 把面板收窄到 `clamp(280px, 48vw, 340px)`（断点与 `lg` 不对齐，是组件自有断点） |

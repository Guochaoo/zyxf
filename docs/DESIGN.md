...
- 大模型配置：头部齿轮**不就地展开表单**，而是打开全局设置弹窗（设置 → 大模型配置）；配置存 localStorage，请求时随 body 下发覆盖服务端 env（服务端已配置时不再下发用户 Key）。设置页右栏：字段分组用浅底色块承载（色块 + 留白分隔，不用描边），输入框为白底 shadow-as-border（`rgba(23,23,23,.08) 0 0 0 1px`）聚焦叠品牌蓝光晕，操作为「次级 `rb-btn-ghost` + 主 `rb-btn-dark`」

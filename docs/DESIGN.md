...
- GitHub Star 按钮（`.sm-gh-star`，`GithubStarButton.jsx`）：仅菜单打开时出现在开关**左侧**（与开关同高 34px、同圆角、同 hover 阴影的一对黑胶囊），含扫光动效与金色星星 hover；star 数在菜单打开时实时请求 GitHub API（失败则隐藏数字块，不缓存失败结果）；点击新窗口打开仓库，点击它**不会**收起菜单（参与 click-outside 豁免，否则按钮被卸载链接点不中）

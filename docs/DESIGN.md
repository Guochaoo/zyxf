...
- 大模型配置：头部齿轮**不就地展开表单**，而是打开全局设置弹窗（设置 → 大模型配置）；配置存 localStorage，请求时随 body 下发覆盖服务端 env（服务端已配置时不再下发用户 Key）。设置页右栏：字段分组为白底（靠留白分隔，不用色块也不用描边），输入框为浅灰底 `#F5F5F5`（暗色改 `var(--field)`）、**无细线描边**、聚焦只叠品牌蓝光晕，操作为「次级 `rb-btn-ghost` + 主 `rb-btn-dark`」

...
- **关于我们 `/about`**：独立页，DM Sans 全页覆盖，**自成一套「直角 + 细边」语言**（标签与 CTA 都是 0 圆角 + `--line-strong` 细边，按钮不遵循 14px 圆角规范）；「加入我们」CTA（`.about-cta`）的 hover 是**斜切扫过**——`::after` 铺 `var(--page)` 底、`skewX(-45deg) scale(0→1)` 0.5s 展开，文字同时翻成 `var(--ink)`（宿主需 `position: relative; z-index: 1; overflow: hidden` 才能让 `z-index:-1` 的伪元素显形；**按钮右侧没有紧贴的箭头方块**）；案例卡是 4:3 直角图块（黑底 + 白信息牌，**图片上没有 hover 遮罩**），页脚是文案 CTA + 活动名跑马灯（`border-t border-line` 分隔，`md` 以上去掉该线）

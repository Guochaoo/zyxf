...
- 键盘可达：按钮 / 链接有 2px Focus Blue 焦点环（**例外**：菜单开关、菜单项、社交链接与设置弹窗里若干按钮显式 `outline: none` 且暂无替代样式，属已知缺口，见 issue #56）；热力图单元格进 Tab 顺序并用 `aria-label` 播报日期与下载数
...
- `--green` #3fae6b（占用/存储绿）：excel 族别徽章、环比上升提示
...
- 颜色随语义：下载 `bg-accent`、上传 `bg-orange`、今日 `bg-red`
- 用于下载热力图、今日动态、下载量、近期上传、近期下载等卡片标题行
...
- **行反馈**：按下 `active:scale-[0.98]`（**按在行内操作按钮上时不缩放**，原因见下条）；键盘焦点 `focus-visible:ring-2 ring-inset ring-ink/30`
...
- **行操作按钮的命中区**：视觉盒/灰底仍是 **24×24**，命中区用 `::after` 外扩到 **28×44**——横向各 2px 是上限（按钮间距 `gap-1` = 4px，各扩一半正好在中线相遇，不会吃掉相邻按钮的盒子），纵向撑满行高。原因：按钮只有 24px 见方而行高 44px，紧贴按钮的那一圈（含灰框圆角**外侧**）原先命中的是**行本身**，点偏一点就从「下载」变成「打开预览 / 进入文件夹」。操作列容器自身也 `self-stretch` 撑满行高，列内点空处一律不落到行上。⚠️ 行按下缩放会把按钮从光标下移开：按钮在行最右端，以**行中心**为原点缩放 0.98 会让它左移约 6.6px，光标抬起时已落在按钮外、click 归给行——所以按在按钮上时行不缩放（`not-has-[button:active]:active:scale-[0.98]`）
...
- GitHub Star 按钮（`.sm-gh-star`，`GithubStarButton.jsx`）：**wrapper 直接子级**（不在面板内），absolute 终位 `left: calc(100% - var(--sm-panel-w) + 2em)`——面板宽抽成 `--sm-panel-w` 变量（面板 / 预层 / 按钮三处同源），与「资料库」等条目左缘对齐且断点自动同步；top 12px 与开关垂直持平；z-index 15（面板 10 之上、header 20 之下）。开合由菜单 gsap 时间线驱动：打开时从开关按钮下方**淡入并向左流出**（起点右缘与开关右缘重合，不从屏幕边缘进场），关闭时流回；关闭态 `opacity: 0` + `pointer-events: none` + `tabIndex: -1`。黑底胶囊 34px / 14px 圆角与开关成对，扫光 + 金星 hover 保留；star 数挂载时请求一次 GitHub API（模块缓存，失败则隐藏数字块）；**≤480px 紧凑形态**只显示图标 + 「GitHub」+ star 数（隐藏 Star on 前缀，媒体规则须放在各 display 规则之后）；在面板外但参与 click-outside 豁免（点击不收起菜单）。**图标 fill 必须直接写在 path 上**——`fill: currentColor` 以未解析关键字继承、在 path 自己的 color 上解析，祖先墨色染色规则会把它染黑（BUG-107/108/109 根因）
- 面板内容自上而下：主导航（资料库 / 统计面板 / 关于我们，14px / 600，**没有 active 态**）+ 社交渠道（Bilibili）+ 底部账号条
- 面板**自身不滚动**（`overflow: hidden`）：滚动条与左右 `2em` 内边距都落在内层 `.sm-panel-inner`（`min-height: 0` 是它能滚的前提）。账号条是 `aside` 的直接子级、在滚动容器之外，所以列表滚动时它常驻贴底；放进滚动容器里则会被滚动条槽挤窄约 15px（实测 `cardRight` 短一截）
- 账号条：**通栏贴底的矩形条**——宽 = 面板宽（`--sm-panel-w`）、左右下三边与面板边缘齐平、无圆角，底色沿用原胶囊的 `--field`；内容贴条的两端（头像左边距 = 图标组右边距 = `1rem`），不再跟着列表的 `2em` 内容栏。未登录时是中立条（图标头像 + 「未登录 / 游客」文案，头像为 34px 纯白圆底 `#ffffff`，暗色下回到 `--inset` 灰底——纯白配上被强制反转的浅色图标会看不见），点整条进登录；已登录显示用户名 + 角色，右侧两个图标操作——齿轮打开全局设置，另一个是登录 / 退出（配色见 §2）
...
- **打开方式**：菜单里的齿轮打开这个全局弹窗，桌面端默认展出**账户信息**（`section ?? 'account'`）；对话卡片（`ChatComposer`）的齿轮则**直接落到「智能对话配置」板块**（`useSettingsRoute().openSettingsAt('ai')`）。它是**真实路由**（`/settings`，手机端二级为 `/settings/:section`），系统返回手势由路由天然接管：`/settings/ai` → 返回 → `/settings`（一级列表）→ 返回 → 打开设置前的页面。打开时把当前页面存进 `history.state.background`，弹窗之外**原页面照常渲染**（`/settings` 深链也能用）；关闭时用背景位置**替换**设置条目，历史里不留残影
- **桌面（>640px）**：卡片固定 **800×580**（视口更小时按 `100vw/vh - 32px` 收窄），圆角 16px、投影见 §2；左右两栏 = **220px 侧栏** + 内容区（标题 56px 右内边距，关闭按钮绝对定位在卡片右上角 10px 处）。侧栏底色是 `--app-sidebar`（#ECECEE，与资料库左栏同一档灰），**不画分割细线**，靠灰底与右栏白底分区；**顶部有「设置」标题**（18px / 600 / +0.01em，与右栏板块标题同一行高、同样 18px 起点，两栏标题对齐），线下是垂直导航（账户信息 / 外观 / 智能对话配置，行尾 ✓ 表示当前板块、› 表示可进入）；条目交互是**单向往白靠**的两档——hover 半档白（`color-mix(in srgb, var(--surface) 55%, transparent)` ≈ #F6F6F7），选中整档白（`var(--surface)`）+ 与 `shadow-btn` 同款的两层提拉投影（和统计面板的时间范围 / 指标胶囊同一套语汇：组内只有选中项浮到表面上），未选中项靠文字色深浅（`--ink-2` → `--ink`）区分。暗色下 `--surface` 比栏底更暗，改用具名白色叠加（hover `rgba(255,255,255,.06)`、选中 `.12`），方向才不会反过来；手机端条目本来就坐在白卡片里，hover / 选中都回到 `--hover` 浅灰块并去掉投影
...
### 资料库之外的页面
- **统计面板 `/dashboard`**：独立页（`mx-auto w-full`），标题行 = 品牌 logo + 「统计面板」+ 右侧时间范围切换与刷新；主体是三段 12 栅格（8/4、7/5、6/6，`gap-3`）：下载热力图 + 类型分布、今日动态 + 下载量、近期上传 + 近期下载
- **关于我们 `/about`**：独立页，DM Sans 全页覆盖，**自成一套「直角 + 细边」语言**（标签是 0 圆角 + `--line-strong` 细边，按钮不遵循 14px 圆角规范）；「加入我们」CTA（`.about-cta`）是**黑底白字 + 无描边**的直角块（去掉描边时把 `px-3 py-2` 补偿成 `px-[13px] py-[9px]`，外框尺寸不变），hover 为**斜切扫过**——`::after` 铺 `var(--surface)`（亮色 = 纯白、暗色 = 面板色）、`skewX(-45deg) scale(0→1)` 0.5s 展开，文字同时翻成 `var(--ink)`；实现约束三条：宿主需 `position: relative; z-index: 1; overflow: hidden` 才能让 `z-index:-1` 的伪元素显形；**横向外扩必须 ≥ 高度的一半**（`left/right: -50%`），否则斜切后左上 / 右下会各露一个露黑的小三角（`-20%` 只够 `H/W ≤ 0.4`）；**按钮右侧没有紧贴的箭头方块**。案例卡是 4:3 直角图块（黑底 + 白信息牌，**图片上没有 hover 遮罩**），页脚是文案 CTA + 活动名跑马灯（`border-t border-line` 分隔，`md` 以上去掉该线）
...
### 类型分布（统计面板首行右卡）
- 结构：标题 + 主数值（当前选中类型的百分比）+ 分段条（`h-9` 圆角药丸、按 `pct` 分宽、选中 1 不透明 / 其余 .58）+ 图例 + 底部说明条（类型名 + 文件数 + 总大小）
- **图例列数按卡片自身宽度降档**（容器查询，`AllocationCard` 根节点 `@container`）：`grid-cols-2 @[280px]:grid-cols-3 @[366px]:grid-cols-4`。阈值来自实测：每项自然宽最宽 80px（`DOCX 6.7%`）、列间距 6px、卡片两侧 `p-3` 共 24px → 4 列需卡片 ≥362px、3 列 ≥276px、2 列 ≥190px。切列数比让文字省略好：只做 `shrink-0` 时类型名会被截成「P… 61.9%」，信息反而更少（BUG-113）
- 图例项内**圆点必须 `shrink-0`**：它是 flex 子项，卡片窄（图例 4 列每列约 66px）时会被一路压到 0px 宽而彻底看不见；名称允许收缩省略、百分比 `shrink-0` 保完整
...
- 行链接指向**所在文件夹**（`folderTarget`），与下载量 / 近期上传一致，不直接触发预览
- 「近期下载」的数据契约（`GET /api/stats` 的 `recent_downloads`）：按 `file_id` **去重**，取每个文件**最近一次**下载时间，按该时间倒序，**最多 8 个**——所以它是「最近被下载过的 8 个文件」，而不是「最近 8 条下载记录」。刻意**不加时间窗**（加了之后低活跃期整卡会空掉）
- 卡片标题徽章：上传 = `bg-orange` + `ArrowUp`，下载 = `bg-accent` + `ArrowDown`（与下载热力图 / 下载量同色系）
...
| Focus（无障碍） | 2px solid hsla(212, 100%, 48%, 1) + 2px offset | 只给按钮 / 链接 / `[role=button]`；`input/textarea/select` 无聚焦效果（无界，见 §2 与 issue #56） |
...
- 不要给表单控件加焦点描边 / 光晕（无界：聚焦态 = 常驻态；可访问性权衡见 issue #56）
...
| `1100px`（`lg`，覆写） | 三栏 docs 布局生效（左栏 250px + 右栏 300px）、Dashboard 12 栅格开始分栏；反向就是「`lg` 以下」——左右栏整栏隐藏（不做内联回退），改用移动端细品牌行（`h-14`）+ 92% 宽的搜索行 + 菜单导航；`StaggeredMenu` 自己也有一层 `@media (max-width: 1100px)` 把面板收窄到 `clamp(280px, 48vw, 340px)`（JS 侧 `useMediaQuery('(min-width: 1100px)')` 与之同源，改断点要三处一起动） |
...
- 列表行按下缩放 `active:scale-[0.98]` 提供触感反馈（按在行内操作按钮上时除外——缩放会把按钮从光标下移开，见 §4「文件列表」）
...
6. 断点是否只用 640 / 768 / 1100（加必要的 `max-[480px]` 特例）？`lg` 以下是否有合理回退？
...
> 维护约定：本规范与代码必须保持一致（见 `AGENTS.md` §1）。改动影响颜色 / 字距 / 圆角 / 断点 / 组件外观时，请同步更新本文档；发现代码偏离规范或规范漏记的偏差，按 `AGENTS.md` §4 开 GitHub issue 而不在本文档里留吐槽。

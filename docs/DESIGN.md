...
- GitHub Star 按钮（`.sm-gh-star`，`GithubStarButton.jsx`）：absolute 挂在**面板内**顶部——top 12px 与开关垂直持平，left = 面板自身字号的 2em（即面板内边距，与「资料库」等条目左缘对齐）；随面板滑动，打开时从开关按钮（header z 更高）下方**从右向左「流出」**、关闭时流回，无独立动画；黑底胶囊 34px / 14px 圆角与开关成对，扫光 + 金星 hover 保留；star 数挂载时请求一次 GitHub API（模块缓存，失败则隐藏数字块）；面板关闭时随 inert 不可聚焦，点击它也不触发 click-outside 收起（在面板内）

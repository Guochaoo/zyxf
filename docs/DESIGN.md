...
- 系统返回（手机边缘左滑 / 浏览器返回键）：设置弹窗打开期间会往历史里压一条**哨兵条目**接管返回手势——**手机端二级页面第一次返回回到一级列表**（内部会补回哨兵，所以再返回一次才关闭设置），**一级列表或桌面端返回则关闭设置**，不再直接退出整个站点；关闭弹窗（✕ / 遮罩 / 返回）时会把哨兵摘掉，避免之后出现一次「按了没反应」的返回。

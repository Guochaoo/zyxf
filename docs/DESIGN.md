...
- **语义视图（头部第 1 个图标按钮，默认开启，状态跨路由保留）**：在目录层级之上叠加一层「按主题聚类」——同主题节点按 idf 加权的名称相似度连**虚线**语义边（`--kg-semantic-line`，0.8px / `3 3`，画在层级实线之下），并按簇上色、用 `forceX/forceY`（强度 0.05）把同簇节点轻拉向簇心；关掉即回到纯目录视图（层级边与节点配色原样恢复）
...
- 图例：画布左上角 `foreignObject`（`--kg-legend-bg` 半透明白 / 暗色半透明黑），每项 = 色点 + 簇标签（簇内覆盖 ≥1/3 成员的共享 token，最多 2 个），只列成员 ≥2 的簇；悬停某项会点亮该簇（其余节点淡到 0.12、语义边 0.06）

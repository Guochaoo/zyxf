...
- 智能对话配置：头部齿轮**不就地展开表单**，而是打开全局设置弹窗（设置 → 智能对话配置）；配置存 localStorage，请求时随 body 下发覆盖服务端 env（服务端已配置时不再下发用户 Key）。设置页右栏：顶部为**配置来源二选一**（小圆点单选：`使用服务器配置` / `使用自定义配置`，原生 radio 去外观后自绘，未选=1px `--line-strong` 环，选中=`--ink` 环 + 实心点）；选「服务器配置」时**整块隐藏**三个字段并只显示 server 提示，选「自定义配置」才显示字段（字段分组为白底、靠留白分隔，输入框为浅灰底 `#F5F5F5`（暗色改 `var(--field)`）、**无细线描边**、**聚焦也不加光晕/描边**，与登录页输入框同款）；「保存」在服务器配置下会清掉本地存储，「恢复默认设置」= 回到服务器配置 + 清空；操作为「次级 `rb-btn-ghost` + 主 `rb-btn-dark`」

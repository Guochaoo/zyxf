...
- 图谱专用：`--kg-label-stroke` #ffffff（亮色下给图谱标签白衬边；暗色为 `transparent`）· `--kg-c1..--kg-c6` / `--kg-cn`（语义簇色板，见 §4 知识图谱）· `--kg-semantic-line` rgba(23,23,23,.22)（语义虚线）· `--kg-legend-bg` rgba(255,255,255,.75)（图例底）
...
- 图谱语义簇色板另有一套暗色值（`--kg-c1..--kg-c6` 提亮降饱和、`--kg-cn` #9a9a9a、`--kg-semantic-line` rgba(255,255,255,.22)、`--kg-legend-bg` rgba(30,30,30,.75)）：深底上高饱和色会「发光」，反而抢结构语义
...
- **三个档位**（头部前三个图标按钮，当前档用 `aria-pressed` + 实底 `--surface` 表示，状态跨路由保留）：
  - **内容视图（默认）**：边来自后端算好的内容向量相似度（`GET /api/index/semantics`，每节点 top-8、后端起连线阈值 0.6），画布只画 0.8 以上的边；**成簇要求「相似度 ≥0.84 且互为 top-4 最近邻」**（两个条件缺一不可，实测只用阈值时最大簇 125 个成员、纯度 18%，加上互为最近邻后最大 17），簇标签取簇内最高频目录名
  - **名称视图**：边来自文件名主题（本地纯函数分词 + idf 加权相似度），簇标签取簇内共享 token
  - **目录视图**：不画语义层，只有文件夹层级
  - 两档语义视图共用同一套渲染（虚线边、簇色、图例、簇心布局），只是边的来源不同；内容档数据未就绪时给「正在读取内容索引…」，索引为空时给「可切到名称视图」的提示，都不是白屏
...
- **语义层渲染**（内容/名称两档共用）：同簇节点用**虚线**语义边相连（`--kg-semantic-line`，0.8px / `3 3`，画在层级实线之下），按簇上色，并用 `forceX/forceY`（强度 0.05）把同簇节点轻拉向簇心
- 簇色板 `--kg-c1..--kg-c6`（亮色 #4a7fb5/#c07a3e/#5e9b62/#a05fa0/#3f9a9a/#c05a66，暗色整体提亮降饱和），低饱和以避免抢过「文件夹实心 / 文件空心」这层结构语义；簇序即色槽序（大的簇先占色），**超出 6 簇用中性 `--kg-cn`**，保证同一主题不会因重排换色
- 图例：画布左上角 `foreignObject`（`--kg-legend-bg` 半透明白 / 暗色半透明黑），每项 = 色点 + 簇标签（名称档取簇内覆盖 ≥1/3 成员的共享 token 最多 2 个；内容档取簇内最高频目录名 1 个），只列成员 ≥2 的簇；悬停某项会点亮该簇（其余节点淡到 0.12、语义边 0.06）
- 节点着色只换颜色、不改形状：文件夹仍实心、文件仍空心
- ⚠️ 实现约束：局部子图每次都从 `fullGraph` 的**拷贝**构建、端点交回字符串 id —— d3-force 会就地改写传入对象（link 端点变节点对象、节点被写 x/y），共享 memo 里的对象会让「第二次进入同一处」的局部子图退化成单节点、度数也记到 `"[object Object]"` 上（BUG-98）。语义边因此**不绑进 forceLink**，只做只读叠加

...
- **两个档位**（头部前两个图标按钮，当前档用 `aria-pressed` + 实底 `--surface` 表示，状态跨路由保留）：
  ...
  - **目录视图**：不画语义层，只有文件夹层级
  - 内容档数据未就绪时给「正在读取内容索引…」，索引为空时给「可切到目录视图」的提示，都不是白屏
...
- **语义层渲染**（内容视图）：同簇节点用**虚线**语义边相连（`--kg-semantic-line`，0.8px / `3 3`，画在层级实线之下），按簇上色，并用 `forceX/forceY`（强度 0.05）把同簇节点轻拉向簇心
...
- 图例：画布左上角 `foreignObject`（`--kg-legend-bg` 半透明白 / 暗色半透明黑），每项 = 色点 + 簇标签（簇内最高频目录名），只列成员 ≥2 的簇；悬停某项会点亮该簇（其余节点淡到 0.12、语义边 0.06）

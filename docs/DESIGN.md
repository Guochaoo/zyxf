...
- 账号条：**通栏贴底的矩形条**——宽 = 面板宽（`--sm-panel-w`）、左右下三边与面板边缘齐平、无圆角，底色沿用原胶囊的 `--field`，内容内缩量保持原样（内层 2em + 原有 0.9rem）。未登录时是中立条（图标头像 + 「未登录 / 游客」文案），点整条进登录；已登录显示用户名 + 角色，右侧两个图标操作——齿轮打开全局设置，另一个是登录 / 退出（配色见 §2）

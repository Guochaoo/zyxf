...
- 设置是**真实路由**（`/settings`，手机端二级页面为 `/settings/:section`）：系统返回手势/返回键由路由天然接管——`/settings/ai` 返回 → `/settings`（一级列表）→ 再返回 → 打开设置前的页面（不再直接退出站点）。打开设置时把当前页面存进 `history.state.background`，页面路由按背景位置渲染，所以弹窗之外**原页面照常存在**（也支持直接访问 `/settings` 深链：背后渲染资料库，不会被通配重定向冲掉 URL）；关闭设置时用背景位置**替换**掉设置条目，历史里不留 `/settings` 残影。
- 智能对话配置：头部齿轮**不就地展开表单**，而是打开全局设置弹窗（设置 → 智能对话配置；弹窗固定 **800×580**，视口更小时按 `100vw/vh - 32px` 收窄，左右两栏 = 220px 侧栏 + 内容区，关闭按钮绝对定位在**卡片右上角**（内容标题左对齐，右上角是空白，标题另留 56px 右内边距兜底）；**≤640px 窄屏（手机）是两级结构**（仿原生 App 设置页）：整屏页面、底色换成 `--page` 灰，分区靠白色圆角卡片表达；顶部条 = 左侧圆形按钮（一级是 ✕ 关闭、二级是 ‹ 返回）+ **居中标题**（一级「设置」、二级为板块名）；**一级只显示设置列表**（一张白卡片里的列表行：图标 + 文案 + 行尾 ✓ 表示当前板块 / › 表示可进入），点行进入二级；**二级显示该板块内容**（一张白卡片）+ 底部操作条白卡片，且板块内容不再往下拆（如「外观」的界面主题 + 字体语言同页）。桌面端不受影响仍是左右分栏——由 `useMediaQuery('(max-width: 640px)')` 决定渲染哪一级（纯样式差异仍走 CSS 媒体查询）；；配置存 localStorage，请求时随 body 下发覆盖服务端 env（服务端已配置时不再下发用户 Key）。设置页右栏：顶部为**配置来源二选一**（小圆点单选，**纵向排列**：`使用服务器配置` / `使用自定义配置`，原生 radio 去外观后自绘，未选=1px `--line-strong` 环，选中=`--ink` 环 + 实心点；**选中只换颜色、不改字重**，避免文字宽度跳动）；选「服务器配置」时**整块隐藏四个字段**并只显示 server 提示，选「自定义配置」才显示字段（字段**自上而下为 API 地址 → API 协议 → API Key → 模型**；分组不再套底色块/内边距——卡片本身白底，套白块只会多出一圈假缩进，改为纯靠留白分组：块间距 20px / 字段间距 14px / 标签到输入框 6px；**除 API 地址给一个通用格式示例 `https://api.example.com/v1` 外，其余字段不给占位文字**——占位容易被误读成已填的值，写具体厂商又会误导；地址与协议同款外观：浅灰底 `#F5F5F5`（暗色改 `var(--field)`）、**无细线描边**、**聚焦也不加光晕/描边**，与登录页输入框同款；协议用**自绘下拉**（按钮触发器 + Level 3 浮层，与「外观」页语言下拉共用 `.settings-popover` / `.settings-popover-option`；**不用原生 `<select>`**——它的 option 列表由系统绘制，跟不上暗色主题与无界风格；字段容器用 `div` 而不是 `label`，否则 label 会把点击转发给触发器，「API 协议」整行都变成可点区域，故只有触发器本身可点、命名靠触发器的 `aria-label`），三项用官方叫法：`OpenAI Chat Completions` / `OpenAI Responses` / `Anthropic Messages`，与 id `openai-completions` / `openai-responses` / `anthropic-messages` 一一对应）；「保存」在服务器配置下会清掉本地存储，「恢复默认设置」= 回到服务器配置 + 清空；两个操作按钮为右栏**底部固定条**（`.settings-actions` 在滚动区之外，始终钉在右下角，「次级 `rb-btn-ghost` + 主 `rb-btn-dark`」；配对展示故条内 ghost 圆角对齐为 14px，其他页面的 ghost 仍是 6px 规范）
...
- Vercel-docs-style **three-column layout** on **browse routes only** (`/`, `/folder/:id`, 以及设置路由 `/settings`、`/settings/:section`——它是覆盖层，背后照常渲染资料库)；every other page (统计 / 关于我们 / 登录) is **standalone** (no fixed rails)

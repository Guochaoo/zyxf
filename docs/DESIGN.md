...
- **保存校验**：选「自定义配置」时，API 地址 / API Key / 模型**三项 trim 后都非空**才写入 localStorage——半份配置 `ChatComposer` 根本不会下发，存下来等于没配置（口径与 `modeOf` / `hasClientCfg` 一致）。没填完点「保存」**不写存储**：表单下方出现一行 `--red` 提示（`role="alert"`），并把缺的字段标签标红（`aria-invalid` + `aria-describedby` 指回该提示），编辑任一字段或切换配置来源即清除。协议有默认值，不参与校验；「使用服务器配置」下保存 = 清掉本地存储，不受校验影响

...
- 图谱专用：`--kg-label-stroke` #ffffff（亮色下给图谱标签白衬边；暗色为 `transparent`）· `--kg-c1..--kg-c6` / `--kg-cn`（内容分类色板，见 §4 知识图谱）
...
- 图谱内容分类色板另有一套暗色值（`--kg-c1..--kg-c6` 提亮降饱和、`--kg-cn` #9a9a9a）：深底上高饱和色会「发光」，反而抢结构语义
...
- 面板内容自上而下：主导航（资料库 / 统计面板 / 关于我们，14px / 600，**没有 active 态**）+ 社交渠道（Bilibili / Email / Wechat）+ 底部账号卡
...
- **两个档位**（头部前两个图标按钮，当前档用 `aria-pressed` + 实底 `--surface` 表示，状态跨路由保留）：
  - **内容视图（默认）**：**不按目录摊开**，用内容语义分类组织——`根 → 学科分类（大类）→ 内容细分 → 文件`。结构由后端算好（`GET /api/index/taxonomy`：大类 = 顶层学科目录，细分 = 该学科目录内按内容向量 k-means），细分名字由 LLM 起一次并缓存（无 LLM 时回落到「文件名里本细分独有的词」，仍起不出就不显示名字）
  - **目录视图**：文件夹层级，与改动前一致
  - 内容档数据未就绪时给「正在读取内容索引…」，索引里没有可用资料时给「可切到目录视图」的提示，都不是白屏
- 节点三类：**学科节点**（`type: 'topic', kind: 'subject'`，复用目录节点 id 以保留下钻）、**细分节点**（`kind: 'cluster'`，id 前缀 `k:`）、**文件节点**；文件夹与文件的外观规则不变（见下）
- ⚠️ **学科节点是否存在只看「该学科目录在不在当前范围内」**，与它的文件在不在范围内无关——首页的局部范围只有目录没有文件，若按文件判据就会一个学科都不显示（首页会直接空掉）。学科节点的 `size` 取分类里该学科的总文件数；细分与文件节点才按「文件是否在范围内」决定，因此**首页是一张干净的学科地图，点进学科才展开细分**
- 空态分两种：整库索引没有可用资料（`kg.contentEmpty`）vs 只是当前位置没有已索引资料（`kg.contentEmptyHere`，小文件夹里很常见）；提示不该把后者说成「索引里没有可用资料」
- **索引进行中的提示**：分类响应带 `pending`（待索引文件数）时，画布右下角显示一行 10px 的「正在建立内容索引（N 个待处理）…」（`--inset` 底、`--ink-2` 字，`pointer-events: none` 不挡交互），并每 5 秒自动重拉（上限 12 次 ≈ 1 分钟，之后交给手动刷新）；`pending` 归零后自动消失。刚上传的资料在向量生成前不在分类里，没有这条提示会被误认为上传失败
- 力导向布局（d3-force）：分类节点半径 `4 + sqrt(成员数) × 1.6`，文件节点按度数 `2 + sqrt(degree) × 2.2`；文件夹实心 `var(--ink)`，文件 `var(--surface)` 填充 + `var(--line-strong)` 描边；当前文件夹多一圈外环（`r + 3`，`var(--ink)`，opacity .5）
- **分类上色**：每个「内容细分」占一个色槽，细分节点实心填充该色、其下文件用同色描边（一组资料在视觉上是一体的）；色板 `--kg-c1..--kg-c6`（亮色 #4a7fb5/#c07a3e/#5e9b62/#a05fa0/#3f9a9a/#c05a66，暗色整体提亮降饱和），低饱和以免抢过「文件夹实心 / 文件空心」这层结构语义；**超出 6 个细分用中性 `--kg-cn`**，保证同一主题不会因重排换色
- **没有图例**（曾有过、已按人类要求去掉）：分类名直接写在节点上——内容档节点少、语义重，标签**默认全部显示**（目录档仍是「放大超过 1.2× 或悬停才显示」），所以不需要图例去对照颜色
- ⚠️ 实现约束：局部子图每次都从 `fullGraph` 的**拷贝**构建、端点交回字符串 id —— d3-force 会就地改写传入对象（link 端点变节点对象、节点被写 x/y），共享 memo 里的对象会让「第二次进入同一处」的局部子图退化成单节点、度数也记到 `"[object Object]"` 上（BUG-98）。分类图同理每次重建，且建图后要**滤掉悬空边**（分类图的边都从根出发，而局部子图里可能没有根节点，d3 会直接抛 `node not found`）
- **局部视图**：当前文件夹 + 直接邻居；内容档下若当前是某个学科目录，则展开它的内容细分。globe 按钮打开全库弹窗（最大 1200×85vh、圆角 14px、投影 rgba(0,0,0,.12) 0 16px 48px）
- hover 高亮该节点、直接邻居，以及「文件 → 细分 → 学科 → 根」这条归属骨架：其余节点透明度 0.12，连线 0.55 / 其余 0.4
- 标签：10px（分类节点 11px / 600 字重），节点上方 6px，带 `--kg-label-stroke` 衬边
- 交互：拖背景平移、滚轮缩放（0.25–2.5）、拖节点、单击（未拖动）进入文件夹 / 预览文件；学科与细分节点点击都进入该学科目录；静止后自动适配视野

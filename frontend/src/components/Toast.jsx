...
/* 顶部通知卡（Toast）：从页面上边缘滑入，停留 duration 后自动淡出，点 × 可提前关闭。
   四种类型：error / success / info / warning，图标与配色对应（样式移植自 uiverse 通知卡）。
   用 Portal 挂到 body——避免落在带 transform 的祖先内导致 fixed 失效（相对容器定位）。
   用法：<Toast type="success" message="已删除" onClose={() => ...} /> */
  ...
  return createPortal(
    ...
    >
      ...
      <div className="toast-card__icon">
        <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" aria-hidden viewBox={kind === 'info' ? '0 0 24 24' : kind === 'warning' ? '0 0 256 256' : '0 0 512 512'}>
      ...
      >
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 15 15" fill="none" aria-hidden>
          <path fill="currentColor" d={CROSS_PATH} clipRule="evenodd" fillRule="evenodd" />
        </svg>
      </button>
    </div>,
    document.body

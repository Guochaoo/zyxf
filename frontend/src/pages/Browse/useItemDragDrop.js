...
export function useItemDragDrop({ data, folderId, sort, isAdmin, setSort, refresh }) {
  ...
  const computeRowZone = useCallback(
    (e, target /* {type:'folder'|'file', id} */) => {
  ...
  const onRowDragLeave = useCallback((target) => {
    setDropZone((p) =>
      p && p.targetType === target.type && p.id === target.id ? null : p
    );
  }, []);

  const buildReorder = useCallback(
    (drag, target, position /* 'before'|'after' */) => {
      if (!data) return null;
  ...
  const onRowDrop = useCallback(
    ...
    [buildReorder, computeRowZone, folderId, refresh, setSort, sort, t]
  );
